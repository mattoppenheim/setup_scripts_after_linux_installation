...
# Usage: ./copy_setup_files_to_remote.sh <remote_username> <remote_ip> <ssh_keypath>
# Example: ./copy_setup_files_to_remote.sh fred 192.168.1.10 ~/.ssh/id_rsa_fred password123
...
# only want to enter the ssh key password once
eval "$(ssh-agent -s)"
...
#
echo "==== ssh'ing into $1@$2 ===="
# sshpass stalls the script
# sshpass -P SSH_KEY_PASSWORD ssh -i $SSH_KEY_PATH $REMOTE_USER@$REMOTE_IP -o StrictHostKeyChecking=no  "mkdir -p $REMOTE_SCRIPTS_PATH"
